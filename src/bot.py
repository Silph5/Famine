...
async def genRoleFactionCode(interaction:discord.Interaction, role_name:str, 
    ...
    cEmbed = discord.Embed(title=f"Code for {faction_name} aligned {Tos2Info.getRoleDisplayName(roleName)}:", description=f"[[#{rID},{fID}]]", color=Tos2Info.getFactionColour(factionName))
...
async def cmdHelp(interaction:discord.Interaction):
    ...
    helpEmbed.add_field(name="/winstatsbucket", value="Shows your highest win achievement for roles in a given bucket. Requires a linked account.\n\n-# Usage example: `/winstatsbucket tpow`", inline=False)
